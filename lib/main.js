...
$start.click(async () => {
    ...
    const solution = await app.compute_route(matrix) // TODO length or BY TIME
    const route = await app.display_route(solution)
    $go_to_map.attr("href", route._results.url)
    $go_to_map.toggleClass("disabled", false)

    app.info(`${app.formatDistance(route._results.length)}: ${solution.map(r => r.name).join(" → ")}`)
...
function save_to_hash() {
    location.hash = `places=${encodeURIComponent($places.val())}&suffix=${encodeURIComponent($place_suffix.val())}`;
}
